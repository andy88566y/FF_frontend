...
def gen(
    ...
    particle_mode_only_defects = api_helper.get_particle_mode_only_defects(inference_result_dir)
    ...
    for missed_defect_per_lot in particle_mode_only_defects:
        for k, v in missed_defect_per_lot.items():
            df.loc[len(df)] = pd.Series({"Lot ID": k, "Particle-mode-only defects": v})
    st.dataframe(df)
