...
def gen(
    ...
    tp_datas, tn_datas, tp_corrections, tn_corrections = intersections
    ...
    with tp_col:
        st.plotly_chart(
            plot_venn(sets=tp_sets, labels=model_names, miss_count=missed_tp_count, title=f"True Defects Venn Diagram (Total: {ans.count(1)})"),
            clear_figure=True,
        )
    with tn_col:
        st.plotly_chart(
            plot_venn(sets=tn_sets, labels=model_names, miss_count=missed_tn_count, title=f"False Defects Venn Diagram (Total: {ans.count(0)})"),
            clear_figure=True,
        )
