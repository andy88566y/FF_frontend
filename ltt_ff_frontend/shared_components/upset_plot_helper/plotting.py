...
from ltt_ff_frontend.shared_components.upset_plot_helper.utils import (
    ...
    individual_set_size,
    get_nodes_and_edges,
    get_sorted_nodes_and_edges,
    get_nonzero_nodes_and_edges,
    get_xaxis_marginal_data,
    get_yaxis_marginal_data,
)
...
def plot_upset(
    ...
    for i, df in enumerate(dataframes):
        ...
        if sorted_x is not None:
            ...
            t, f, edges = get_sorted_nodes_and_edges(t=t, f=f, edges=edges, sorted_sequence=_sorted_sequence)
        ...
        if len(marginal_data) != 0:
            for idx, data in enumerate(marginal_data):
                ...
                fig.add_trace(
                    ...
                    row=mx_r + len(marginal_data) - idx - 1,
                    col=mx_c,
                )
                ...
                if marginal_y:
                    ...
                    fig.update_yaxes(tickvals=np.arange(0, len(sets)), ticktext=sets, row=id_r, col=mx_c + idx + 2)
        ...
        for i in range(len(sets)):
            if i % 2 == 0:
                fig.add_hrect(
                    y0=(i - 0.5), y1=(i + 0.5), layer="below", fillcolor="#EBEBEB", line_width=0, row=tf_r, col=tf_c
                )
