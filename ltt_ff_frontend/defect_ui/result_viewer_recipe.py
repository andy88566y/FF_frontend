...
def calculate_recipe_filtered_results(output_dir: str, recipe: dict[str, Any]) -> dict[str, Any]:
    ...
    prediction_list = [0] * len(answer_list)

    positive = answer_list.count(1)
    negative = answer_list.count(0)
    unlabeled = answer_list.count(-1)
    true_positive = sum(1 for pred, ans in zip(prediction_list, answer_list)
                         if pred == 1 and ans == 1)
    false_positive = sum(1 for pred, ans in zip(prediction_list, answer_list)
                         if pred == 1 and ans == 0)
    true_negative = sum(1 for pred, ans in zip(prediction_list, answer_list)
                         if pred == 0 and ans == 0)
    false_negative = sum(1 for pred, ans in zip(prediction_list, answer_list)
                         if pred == 0 and ans == 1)
    filtered_unlabeled_defect_count = sum(1 for pred, ans in zip(prediction_list, answer_list)
                                     if pred == 1 and ans == -1)
...
def app() -> None:
    ...
    if rv_output_dir not in invalid_input:
        ...
        with st.expander(label="LRF ClassType count"):
            defects = helper.get_lrf_data(output_dir=rv_output_dir,
                                            cols=["ClassType"],
                                            include_prob=False)
