...
def app(output_dir_default: str, rv_m1_output_dir: str, rv_m2_output_dir: str, st_gen_lrf_type: str) -> None:
    ...
    if rv_m1_output_dir not in invalid_input and rv_m2_output_dir not in invalid_input:
        ...
        for raw_data_1, meta_1, raw_data_2, meta_2 in zip(
            sorted(model_1_raw_data[2]), model_1_metadata, sorted(model_2_raw_data[2]), model_2_metadata
        ):
        ...
        model_2_name = model_2_metadata[0].get("model_name", model_2_metadata[0].get("model_name_0", ""))
        model_2_threshold = model_2_metadata[0].get("model_threshold", model_2_metadata[0].get("model_threshold_0", ""))
        ...
        for index, (meta_1, meta_2) in enumerate(zip(model_1_metadata, model_2_metadata)):
            if st_gen_lrf_type == "top_k":
                ...
                m1_current_threshold = helper.get_topk_model_threshold(
                    output_dir=rv_m1_output_dir, top_k=st.session_state["m1_topk"], lot_id=meta_1["lot_id"]
                )
                if m1_current_threshold < rv_m1_threshold:
                    rv_m1_threshold = m1_current_threshold

                m2_current_threshold = helper.get_topk_model_threshold(
                    output_dir=rv_m2_output_dir, top_k=st.session_state["m2_topk"], lot_id=meta_2["lot_id"]
                )
        ...
        with vr3_col1:
            st.plotly_chart(
                result_viewer.generate_2D_plot(
                    m1_aggregated_data_lists, m2_aggregated_data_lists, rv_m1_threshold, rv_m2_threshold
                )
            )
        ...
        with vr3_col2:
            ...
            if 1 not in set(m1_aggregated_data_lists[2]) or 1 not in set(m1_aggregated_data_lists[2]):
                ...
                st.plotly_chart(
                    result_viewer.plot_multilot_roc(
                        [
        ...
        for index, meta in enumerate(model_1_metadata):
            if st_gen_lrf_type == "top_k":
                ...
                current_threshold = helper.get_topk_model_threshold(
                    output_dir=rv_m1_output_dir, top_k=st.session_state["m1_topk"], lot_id=meta["lot_id"]
                )
