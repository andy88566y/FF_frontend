    ...
    while i < len(lines):
        ...
        while i < len(lines) and not re.match(r"^\d+$", lines[i]):
            ...
            if rs:
                ...
                raise ValueError(f"⚠️ Could not parse recipe line: {lines[i]}")
            i += 1
        config_data.setdefault(layer, {}).setdefault(str(week), {})[site] = recipes


def update_config_by_yaml(config_data: dict, recipe_yaml: Any) -> None:
    file_name = recipe_yaml.name
    update_data = yaml.safe_load(recipe_yaml)
    match = re.match(r"^(W\d+)_([a-zA-Z0-9]+)_([a-zA-Z0-9]+)$", file_name)
    if not match:
        raise ValueError(f"Invalid filename format: {file_name}. Expected format: WX_layer_site.yaml")
    week, layer, site = match.groups()
    recipe_formated = [tuple(model.values()) for model in update_data["recipes"]]

    config_data.setdefault(layer, {}).setdefault(week, {})[site] = recipe_formated
